//
//  APINinjasGeocoderServiceTests.swift
//  MiniWeatherTests
//
...
final class APINinjasGeocoderServiceTests: XCTestCase {
    ...
    func test_getLocationsNamed_LocationsReturnedSuccessfully() async throws {
        ...
        // When
        let sut = APINinjasGeocoderService<MockLocation, MockPartialLocation>(parser: parser, networkService: networkService, apiKeysProvider: provider, geocodingRequest: mockRequest)
    ...
    func test_getLocationAt_LocationsReturnedSuccessfully() async throws {
        ...
        let url = try mockRequest.createURLRequest().url!
        MockURLProtocol.stub(url: url, data: try JSONEncoder().encode([MockPartialLocation()]), response: HTTPURLResponse(url: url, statusCode: 200, httpVersion: nil, headerFields: nil), error: nil)
        let coordinates = CLLocationCoordinate2D(latitude: 0, longitude: 0)
        
        // When
        let sut = APINinjasGeocoderService<MockLocation, MockPartialLocation>(parser: parser, networkService: networkService, apiKeysProvider: provider, reverseGeocodingRequest: mockRequest)
