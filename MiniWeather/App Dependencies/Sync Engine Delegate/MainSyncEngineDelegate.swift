//
//  MainSyncEngineDelegate.swift
//  MiniWeather
//
//  Created by Ezenwa Okoro on 11/05/2024.
//
        ...
        Task { [self] in
            ...
            for modification in event.modifications {
                ...
                do {
                    ...
                    if let itemRecord = records.first(where: { $0.recordID.recordName == id }) {
                        ...
                        logger.log("Received location modification: \(record.recordID.recordName)")
                    } else {
                        guard var item = Location(record: record) else {
                            continue
                        }
                        recordsToAdd.append(record)
                        itemsToAdd.append(item)
                        logger.log("Received location addition: \(record.recordID.recordName)")
                    }
                } catch {
            ...
            for deletion in event.deletions {
                ...
                do {
                    ...
                    logger.log("Received location deletion: \(deletion.recordID.recordName)")
                } catch {
                    logger.error("Failed to update recordsToDelete: \(error)")
                    continue
                }
            }
